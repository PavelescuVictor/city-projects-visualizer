...
const allStatuses = Object.keys(statusMeta) as ProjectStatus[];
...
function createProjectId(name: string, projects: DevelopmentProject[]) {
...
export default function App() {
  const [projects, setProjects] = useState<DevelopmentProject[]>(canEdit ? [] : initialProjects);
  const projectsRef = useRef<DevelopmentProject[]>(canEdit ? [] : initialProjects);
  const savedProjectsRef = useRef<DevelopmentProject[]>(canEdit ? [] : initialProjects);
  ...
  const [focusRequest, setFocusRequest] = useState({ projectId: "", requestId: 0 });
  ...
  const [projectPendingDelete, setProjectPendingDelete] = useState<DevelopmentProject | null>(null);
  ...
  useEffect(() => {
    ...
    fetch("/api/projects", { cache: "no-store" })
      ...
      .then((projectData: DevelopmentProject[]) => {
  ...
  const clearProjectSelection = useCallback(() => {
    ...
    setIsEditMode(false);
    setFocusRequest((current) => ({ projectId: "", requestId: current.requestId }));
  }, []);
  ...
  const discardUnsavedEditChangesBeforeSelection = useCallback((nextProjectId: string) => {
  ...
  const handleProjectSelect = useCallback((project: DevelopmentProject) => {
    closeCreateMode();
  ...
  const handleProjectFocus = useCallback((project: DevelopmentProject) => {
    closeCreateMode();
    discardUnsavedEditChangesBeforeSelection(project.id);
  ...
  const handleProjectToggleFocus = useCallback((project: DevelopmentProject) => {
    closeCreateMode();
  ...
  const handleProjectEdit = useCallback((project: DevelopmentProject) => {
    if (!canEdit) {
      return;
    }
    ...
    closeCreateMode();
    setSelectedProjectId(project.id);
    setIsEditMode((current) => (current && selectedProjectIdRef.current === project.id ? false : true));
  }, [closeCreateMode]);
  ...
  const handleProjectChange = useCallback((updatedProject: DevelopmentProject) => {
    if (!canEdit) {
      return;
    }
  ...
  const handleProjectDelete = useCallback(async (projectToDelete: DevelopmentProject) => {
    if (!canEdit) {
      return;
    }
    ...
    try {
      ...
      const savedProjects = (await response.json()) as DevelopmentProject[];
      ...
      {canEdit && projectPendingDelete ? (
        <div className="modal-backdrop" role="presentation">
          <div className="confirm-modal" role="dialog" aria-modal="true" aria-labelledby="delete-project-title">
            ...
            <p>
              This removes <strong>{projectPendingDelete.name}</strong> from the local project list.
            </p>
            <div className="modal-actions">
              <button className="modal-secondary-button" type="button" onClick={() => setProjectPendingDelete(null)}>
                Cancel
              </button>
              <button
                ...
                type="button"
                onClick={() => {
                  void handleProjectDelete(projectPendingDelete);
