...
const MapFilterToggles = (props: MapFilterTogglesProps) => {
	const { showParcels, showMarkers, onShowParcelsChange, onShowMarkersChange } = props;
	const [isExpanded, setIsExpanded] = useState(false);

	return (
		<section className={`map-layer-toggles${isExpanded ? " is-expanded" : ""}`} aria-label="Map layer toggles">
			<button
			...
			>
				<ListFilter
					className="map-layer-button-icon map-layer-button-icon-filter"
					size={20}
					aria-hidden="true"
				/>
